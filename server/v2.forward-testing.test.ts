...
import { forwardTestVerdictEligibility } from "./v2/courtroom";
...
assert.equal(svc.create({ ...request, courtVerdict: "approve_for_replay" as const }).record?.status, "monitoring");
assert.equal(svc.create({ ...request, courtVerdict: "reject" as const }).record, null);
assert.equal(svc.create({ ...request, courtVerdict: "watch" as const }).record, null);
assert.equal(svc.create({ ...request, courtVerdict: "revise" as const }).record, null);
assert.equal(svc.create({ ...request, courtVerdict: "unknown" as never }).events[0].payload.reason, "unknown_court_verdict");
assert.deepEqual(forwardTestVerdictEligibility("approve_for_replay"), { eligible: true, normalizedVerdict: "approve_for_forward_test", reason: "historical_replay_approval_compatible" });
assert.deepEqual(forwardTestVerdictEligibility("approve_for_forward_test"), { eligible: true, normalizedVerdict: "approve_for_forward_test", reason: "approved_for_forward_test" });
assert.equal(forwardTestVerdictEligibility("reject").eligible, false);
