...
const env = { FINCOACH_DEMO_BROKER_EXECUTION_ENABLED: "true", FINCOACH_LIVE_EXECUTION_ENABLED: "false", FINCOACH_PAPER_EXECUTION_ENABLED: "false", FINCOACH_PORTFOLIO_LIVE_EXECUTION_ENABLED: "false", OANDA_ENV: "practice", OANDA_BASE_URL: "https://api-fxpractice.oanda.com/v3" };
...
const promotion = { promotionId: "promotion-1", strategyId: "strategy-1", strategyVersion: 1, authorizedForPractice: true as const, status: "active" as const, environment: "practice" as const, authority: "operator", approvedBy: "operator", approvedAt: now.toISOString(), reason: "validated", evidenceEventIds: ["promotion-source"], lifecycleDecisionId: "decision-1", forwardTestIds: ["forward-1"], evaluationIds: ["evaluation-1"], policyVersion: "test", idempotencyKey: "promotion-1", correlationId: "correlation-1", causationId: "causation-1", supersedesPromotionId: null, lineageEventIds: ["promotion-source"] };
...
const revoked = await new V2OandaPracticeExecutionBridge(new InMemoryV2ExecutionRequestRepository(), broker).process({ signal: { ...signal, signalId: "revoked-promotion" }, strategy, forwardTest, lifecycle: { decisionId: "decision-1", toState: "candidate" }, promotion: { ...promotion, authorizedForPractice: false, status: "revoked" }, killSwitchActive: false, practiceCapacityAvailable: true, env, now });
assert.equal(revoked.eligibility.reason, "research_only_without_explicit_promotion");
const nonResearchFlagged = await new V2OandaPracticeExecutionBridge(new InMemoryV2ExecutionRequestRepository(), broker).process({ signal: { ...signal, signalId: "no-promotion" }, strategy: { ...strategy, researchOnly: false }, forwardTest, lifecycle: { decisionId: "decision-1", toState: "candidate" }, killSwitchActive: false, practiceCapacityAvailable: true, env, now });
assert.equal(nonResearchFlagged.eligibility.reason, "research_only_without_explicit_promotion");
