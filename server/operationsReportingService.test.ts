...
const performance = await service.telegramMessage("/performance", "", now);
...
assert.match(performance, /BACKTEST PERFORMANCE/);
assert.match(performance, /FORWARD-TEST PERFORMANCE/);
assert.match(performance, /DEMO\/PRACTICE BROKER PERFORMANCE/);
assert.match(performance, /LIVE PERFORMANCE/);
assert.doesNotMatch(performance, /Insufficient evidence to estimate profitability/);
