...
Real-provider fallback is allowed and observable. For example, if Twelve Data is primary and Alpha Vantage supplies real data after Twelve Data is rate-limited, returned quotes/bars carry `marketData.dataKind=REAL_PROVIDER_FALLBACK` and a deduplicated operator incident is recorded. Fixture, simulated, or synthetic data is not a silent substitute for execution-capable or performance-evaluation workflows; those workflows block/degrade and emit an operator alert requiring a real provider.
